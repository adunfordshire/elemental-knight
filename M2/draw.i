...
void drawPlayer(OBJ_ATTR shadowOAM[], PLAYER * player) {
    ...
    shadowOAM[PLAYERSPRITE].attr1 = (1 << 14) | (0x1FF & player->screenCol);
    shadowOAM[PLAYERSPRITE].attr2 = ((player->element * 6) + player->currFrame * 2)*32+(player->dir * 2);
}
...
void drawEnemy(OBJ_ATTR shadowOAM[], ENEMY enemy[], int enemyLen) {
    for (int i = 0; i < enemyLen; i++) {
        if (enemy[i].active) {
            ...
            shadowOAM[ENEMYSPRITE + i].attr2 = (0)*32+((enemy[i].element + 4) * 2);
        } else {
